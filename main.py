...
class VoiceBot:
    ...
    def _is_default_voice(self, key: str) -> bool:
        if key in DEFAULT_VOICES:
            return True
        if VOICE_ARTISTS.get(key, {}).get("is_default"):
            return True
        ref_id = VOICE_ARTISTS.get(key, {}).get("reference_id")
        default_ref_ids = {v["reference_id"] for v in DEFAULT_VOICES.values()}
        return ref_id in default_ref_ids
    ...
    def _prune_pending_downloads(self, max_age_seconds: int = 3600):
        now = datetime.now().timestamp()
        stale = [
            k
            for k, v in self.pending_downloads.items()
            if now - v["ts"] > max_age_seconds
        ]
        for k in stale:
            self.pending_downloads.pop(k, None)

    def _download_keyboard(self, file_id: str, caption: str) -> InlineKeyboardMarkup:
        token = f"dl_{file_id}"
